function checkDerivativeNumerically(f, x, dx)
...
%   its tensor input X and the derivative DX at X and compares DX to
%   a numerical approximation of the derivative reutring their difference
%   ERR.
...
for k = 1:size(x,3)
  for j = 1:size(x,2)
    for i = 1:size(x,1)
      xp = x ;
      xp(i,j,k,:) = xp(i,j,k,:) + delta ;
      yp = f(xp) ;
      dx_numerical(i,j,k,:) =  (yp - y) / delta ;
    end
  end
end
err = dx_numerical - dx ;
