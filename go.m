...
%imdb.images.image = bsxfun(@minus, imdb.images.image, mean(imdb.images.image,4)) ;
...
net = cnn_train(net, imdb, @getBatch,  ...
  'errorFunction', 'none', ...
  'batchSize', 16, ...
  'learningRate', [0.001, 0.2*ones(1,4), 0.1*ones(1,3), 0.01*ones(1,3)], ...
  'numEpochs', 11, ...
  'continue', false, ...
  'expDir', 'data/exp17', ...
...
for k = 1 : numSamples
  ...
  clf ;
  subplot(2,3,1) ; imagesc(im) ; axis image ; colorbar ;
  subplot(2,3,2) ; imagesc(label) ; axis image ; colorbar ;
  subplot(2,3,3) ; imagesc(res(end-1).x) ; axis image ; colorbar ;
  subplot(2,3,4) ; imagesc([label, res(end-1).x, abs(res(end-1).x-label)]) ; axis image ; colorbar ;
